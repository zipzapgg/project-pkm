        ...
        try:
            ...
            try:
                ...
                if conn:
                    cur = conn.cursor()
                    sql_query = """
                        INSERT INTO siswa (nama, nilai_mtk, nilai_bhs, nilai_bing, nilai_ipa, 
                                           minat_logika, minat_sosial, minat_kreatif, minat_bahasa, hasil_jurusan)
                        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                    """
                    sql_values = (
                        nama, mtk, bindo, bing, ipa_ips,
                        minat_logika, minat_sosial, minat_kreatif, minat_bahasa,
                        hasil_string
                    )
            ...
            print(f"Terjadi kesalahan saat kalkulasi fuzzy: {e}")
            flash("Terjadi kesalahan internal saat memproses data Anda. Silakan coba lagi.", "error")
            return redirect(url_for('tes'))
...
def admin():
    ...
    try:
        ...
        if conn:
            ...
            cur.execute("""
                SELECT id, nama, nilai_mtk, nilai_bhs, nilai_bing, nilai_ipa, 
                       minat_logika, minat_sosial, minat_kreatif, minat_bahasa, hasil_jurusan 
                FROM siswa 
                ORDER BY id DESC
            """)
